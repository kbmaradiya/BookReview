...
async function prepare(){
    await db.query(sql`CREATE TABLE book_review (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, review VARCHAR NOT NULL,
        rating INTEGER NOT NULL, book_id INTEGER NOT NULL,
        FOREIGN KEY(book_id) REFERENCES book(id));`);
}

const prepared = prepare();

async function set(review, rating, book_id){
    await prepared;
    await db.query(sql`INSERT INTO book_review (review, rating, book_id)
    VALUES (${review}, ${rating}, ${book_id});`);
}
